...
Weapon::Weapon(char* name, char* spritePath) : Entity(50, 50, 200, 50) {
    this->name = name;
...
void Weapon::display(Entity* e) {
    ...
    rotation = atan2( (float)(mouseY - e->getPosY() + 20), (float)(mouseX - e->getPosX())) * 180 / 3.14;
    ...
    Rectangle destRec = { (float)e->getPosX(), (float)e->getPosY() + 20, (float)width, (float)height };
